...
contract TestMargin is Base {
    ...
    function testDeposit(uint256 delRisky, uint256 delStable) public returns (Margin.Data memory) {
    ...
    /// @param  delStable  The amount of stable tokens to add to margin
    /// @return The margin data storage item
    function testWithdraw(uint256 delRisky, uint256 delStable) public returns (Margin.Data memory) {
        uint128 preX = margins[msg.sender].balanceRisky;
        uint128 preY = margins[msg.sender].balanceStable;
